...
> **注意（dev 環境）:** dev 環境は毎日 JST 0:00 に自動シャットダウン（desiredCount: 0）される。
> シャットダウン後にデプロイしても ECS タスクは起動しないため、サイトは 503 のまま。
> デプロイ後に dev 環境を確認したい場合は、[dev-startup.md](./dev-startup.md) の手順で起動すること。
